...
> Vendored from `forrestchang/andrej-karpathy-skills` / footprint project's
> Cursor rule (`.cursor/rules/front-end-guidelines.mdc`) on 2026-05-04 to
> guarantee portability. The full version is checked in here so any IronSpot
> clone gets the same baseline regardless of the user's machine layout.

This document summarizes key frontend design principles and rules, showcasing
recommended patterns. Follow these guidelines when writing frontend code.

## How this fits with IronSpot's review setup

- A short reminder (4 principles + key gap patterns) is in `src/CLAUDE.md`,
  which auto-loads on every entry into `src/`.
- A PostToolUse hook (`.claude/hooks/frontend-guidelines-reminder.sh`)
  re-injects the reminder after every Edit/Write of frontend code, so the
  rules stay primed during long sessions.
- The `ff-review:review` skill catches violations at task boundary (PR
  creation) via `/verify` step 4.

When in doubt about a specific pattern below, search for the keyword in the
code-review chat history; PR #17's `ff-review:review` run is a worked example.
