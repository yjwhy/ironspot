...
- **Moderation analytics + ops dashboard** (E + C) — Phase 5 hypothesis H1 ("auto-ban thresholds catch real bad actors without false-banning newcomers") measurement enablement plus a unified web dashboard consuming both D and E. SQL view + admin endpoint over the existing `reports` table (no new schema), Slack weekly digest at Monday 09:00 KST, and a Spring Boot static HTML dashboard at `/admin/dashboard.html` behind Basic Auth. See **Moderation analytics + dashboard plan** section below for the full grill outcome and the 5-slice implementation plan.
...
### Moderation analytics + dashboard plan

#### Why now

Phase 5 hypothesis H1 ("auto-ban thresholds 3 actioned / 5 dismissed catch real bad actors without false-banning newcomers") is undecidable without per-user disposition distribution data plus a ban event audit trail. D enabled H2 (NL search caching feasibility) with the same pre-launch ship rationale; E mirrors that pattern for the moderation side. Cost is 0원 (no new schema — reuses existing `reports` table; one new env var). Bundled with **C (Spring Boot ops dashboard)** because the dashboard is the unified consumption surface for both D and E — operators currently have only Supabase SQL Editor + curl to read D's analytics, and shipping E without the dashboard would repeat that gap.

#### Locked decisions (grilled 2026-05-19)

| Q   | Decision                                                                                                              | Why                                                                                                                                                                                                                                                                  |
| --- | --------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| E1  | Reuse existing `reports` table; no new schema                                                                         | Disposition counts already derivable via `reportRepository.countActionedByUploader` / `countDismissedByReporter` (Task 34). Schema-free saves a migration.                                                                                                           |
| E2  | Time window: admin endpoint takes period param `7d` / `30d` / `all`                                                   | `all` (since-launch) only added for E (not D) because ban event accumulation is meaningful as a cumulative log, unlike rolling query frequency.                                                                                                                      |
| E3  | SQL view: single `moderation_analytics_30d` flat shape                                                                | One row per user with non-zero disposition activity OR `banned_at IS NOT NULL`. Columns: user_id, role, banned_at, actioned_against_uploader, dismissed_by_reporter. Aggregations (histogram buckets, top-N reporters) computed in the admin endpoint, not the view. |
| E4  | Slack weekly digest: Monday 09:00 KST → `#ironspot-moderation`                                                        | Push-based for "operator should look at this" cadence. Daily would be noise pre-launch (zero data). Reuses `AdminNotificationService.post` (Phase 2).                                                                                                                |
| E5  | Pre-launch ship as Phase 4 Operational item (not numbered Task)                                                       | Same rationale as D: 4-8 week earlier H1 decidability + small scope (5 slices).                                                                                                                                                                                      |
| C1  | Auth: Basic Auth via env var `DASHBOARD_PASSWORD`, separate Spring Security chain for `/admin/dashboard/**`           | Beats JWT-paste on ease (1 password manager save vs paste-per-session) and beats JWT-paste on security (no JS-accessible credential, no XSS exfil surface). HTTPS-only Render in transit. Fail-fast: startup fails if env var missing.                               |
| C2  | Rendering: vanilla HTML + JS, no framework, no Thymeleaf                                                              | 0 dependencies. Single static file ≤ 250 lines. Spring Boot serves from `src/main/resources/static/admin/dashboard.html`.                                                                                                                                            |
| C3  | Data endpoint: aggregated `GET /admin/dashboard/data?period=...` returning combined D + E JSON                        | One round-trip per page load. Internal delegates to existing `AdminService` methods.                                                                                                                                                                                 |
| C4  | Layout: single page, 3 sections (NL Search top-N / Moderation distribution / Ban event log), `<select>` period switch | Pull-only, manual refresh button + on-period-change. No polling, no charts (Chart.js dependency rejected).                                                                                                                                                           |
| C5  | InMemoryUserDetailsManager with single `admin` user + BCrypt password from env                                        | One-operator pre-launch, scope-appropriate. Phase 5 standalone web UI (item 6) replaces with proper OAuth.                                                                                                                                                           |

#### Slices

| Slice | Files                                                                                                                                                                                                                                                                                                                                                                                                | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| ----- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| E1    | `iron-spot-api/src/main/resources/db/migration/V4__moderation_analytics_view.sql` (new), `iron-spot-api/src/test/resources/init-test-db.sql`, `iron-spot-api/src/main/java/com/ironspot/admin/ModerationAnalyticsRepository.java` (new), `docs/plans/phase-4/implementation.md` (this section), `docs/plans/phase-4/PROGRESS.md`                                                                     | Plan entry. SQL view `moderation_analytics_30d` (single flat row per user with non-zero activity). Repository methods: `distribution(periodDays)`, `topReporters(periodDays, limit)`, `banEvents(periodDays)`.                                                                                                                                                                                                                                                                                                                            |
| E2    | `iron-spot-api/src/main/java/com/ironspot/admin/AdminController.java` (extend), `iron-spot-api/src/main/java/com/ironspot/admin/AdminService.java` (extend), `iron-spot-api/src/main/java/com/ironspot/admin/dto/ModerationAnalyticsResponse.java` (new), `iron-spot-api/src/test/java/com/ironspot/admin/AdminControllerIT.java` (extend), `openapi.json`, `src/shared/generated/...` (Orval regen) | `GET /api/admin/moderation-analytics?period=7d                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | 30d | all`returning`{period, dispositionHistogram, topReporters, banEvents}`. IT: admin 200 / user 403 / anonymous 401 / invalid period 400. OpenAPI + Orval regen. |
| E3    | `iron-spot-api/src/main/java/com/ironspot/admin/ModerationDigestJob.java` (new), `iron-spot-api/src/test/java/com/ironspot/admin/ModerationDigestJobIT.java` (new)                                                                                                                                                                                                                                   | `@Scheduled(cron = "0 0 9 ? * MON", zone = "Asia/Seoul")` aggregates 7-day window via `ModerationAnalyticsRepository`, formats Korean Slack message, posts via `AdminNotificationService.post`. IT: 2 cases (digest formatting, idempotent re-run).                                                                                                                                                                                                                                                                                       |
| C1    | `iron-spot-api/src/main/java/com/ironspot/common/security/DashboardSecurityConfig.java` (new), `iron-spot-api/src/main/java/com/ironspot/admin/DashboardController.java` (new), `iron-spot-api/src/main/resources/static/admin/dashboard.html` (new), `iron-spot-api/src/test/java/com/ironspot/admin/DashboardSecurityIT.java` (new)                                                                | Separate `SecurityFilterChain` bean ordered before the JWT chain, matches `/admin/dashboard/**`, requires HTTP Basic auth via `InMemoryUserDetailsManager` keyed off `DASHBOARD_PASSWORD` env. `DashboardController.GET /admin/dashboard/data?period=...` aggregates both `AdminService.getNlSearchAnalytics` + `AdminService.getModerationAnalytics`. Static HTML uses vanilla JS to fetch + render. Startup fail-fast when env missing. IT: 401 without creds, 401 wrong password, 200 with correct creds, JSON shape on data endpoint. |
| E4    | `docs/harness/operations.md`, `docs/plans/phase-4/PROGRESS.md`, `docs/launch/pre-submission-checklist.md` (operational section)                                                                                                                                                                                                                                                                      | Operations entry: how to set `DASHBOARD_PASSWORD` on Render, dashboard URL, Slack digest channel + cadence. PROGRESS Operational row closes. Pre-submission checklist Section 7 gains the new env var.                                                                                                                                                                                                                                                                                                                                    |

#### Verification

1. `./gradlew test` — backend ITs all pass including +4 (E2 admin endpoint) + 2 (E3 digest) + 4 (C1 dashboard auth + data) = +10 cases.
2. `pnpm lint && pnpm exec tsc --noEmit && pnpm jest` — frontend touches only Orval-generated client (E2). No hand-written FE.
3. `/verify` slash command at task boundary (FE touched via Orval regen).
4. Post-merge live: set `DASHBOARD_PASSWORD` on Render, browse to `https://ironspot.onrender.com/admin/dashboard.html`, confirm Basic Auth prompt + dashboard renders. Trigger one NL search + one report, observe row updates in dashboard.

#### Token cost

- Groq/Gemini LLM: 0 (no LLM in path).
- Sentry: weekly Slack digest emits 1 INFO event per week (52/year), under 5000/month free tier.
- Render: ~0 compute delta (one new `@Scheduled` weekly job, two new endpoints, one static resource).
- Slack: 1 message per week to `#ironspot-moderation`.

#### Rollback

Schema-free, so rollback drops to file-deletion + revert. Spring Security chain order matters — if `DashboardSecurityConfig` introduces a chain ordering bug that affects the JWT chain, immediate revert is safe. `DASHBOARD_PASSWORD` env can be left set even after rollback (no consumer).
