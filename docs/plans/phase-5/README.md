...
- [ ] Add an "undo" toast on the camera screen for ~5 s ("○○를 등록했어요 · 취소") that rolls back the gym row if tapped. Persisted state hand-over via expo-router params, not a global store. → Backend half **shipped** (item 14a: `DELETE /api/gyms/{id}` + V9 `created_by_user_id` column + 7-case authorisation IT). Frontend toast still pending — needs visual sim verification (deferred from Phase A overnight scope since `pnpm snap` review is a user-in-the-loop step).
