...
### From Task 34 (Admin in-app surface)

- **Admin entry point in 마이 tab** — Task 34 ships the `/admin/queue` + `/admin/photo/{id}` screens behind `AdminGuard`, but no in-app navigation surfaces them. Admins currently reach the queue only via deep link (`ironspot:///admin/queue`), so the role is functionally invisible inside the app. Phase 4 should add a conditional row in `AuthenticatedProfile` (between "내가 추천한 사진" and "계정 설정") that renders only when `user.role === 'admin'` and routes to `PROFILE_ROUTES.adminQueue` (new). Skip if the **standalone admin web UI** (separate Task 27 carry-over above) ships first — that supersedes the in-app entry. Trade-off note: deep-link-only access is acceptable for a one-person admin team where the moderator already knows the URL; the in-app entry only matters once a second admin is added.
